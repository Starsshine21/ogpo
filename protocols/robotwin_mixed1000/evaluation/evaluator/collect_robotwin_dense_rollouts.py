...
ROBOTWIN_ROOT = Path(
    os.environ.get("ROBOTWIN_ROOT", "external/RoboTwin")
)
OGPO_SOURCE_ROOT = Path(
    os.environ.get("OGPO_ROOT", str(Path(__file__).resolve().parents[4]))
) / "src"
...
def main() -> None:
    ...
    parser.add_argument(
        "--pi05-checkpoint-dir",
        type=Path,
        default=Path("<PI05_ROOT>/model_clean50"),
    ...
    if args.ogpo_checkpoint is not None:
        ...
        sys.path.insert(0, str(OGPO_SOURCE_ROOT))
