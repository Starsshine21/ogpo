...
def make_rankq_actions(
    ...
    *,
    ...
    random_normalized = random_unit * 2.0 - 1.0
...
def rank_pair_loss(q_positive: torch.Tensor, q_negative: torch.Tensor) -> torch.Tensor:
    """Vanilla RankQ pair loss with no margin or temperature."""
...
def compute_rankq_loss(
    ...
    """Compute independent per-head vanilla RankQ losses on decoded Q expectations."""
    ...
    success_pair_losses = torch.stack(
        [rank_pair_loss(q_positive, q_negative) for q_positive, q_negative in relations.values()],
        dim=1,
    )
    success_loss_per_head = _masked_success_pair_sum(success_pair_losses, success_mask)
    failure_pair_loss = rank_pair_loss(positive, q_values["random"])
    failure_loss_per_head = _masked_head_mean(failure_pair_loss, failure_mask)
    loss_per_head = success_loss_per_head + failure_loss_per_head
    ...
    for name, (q_positive, q_negative) in relations.items():
        ...
        metrics[f"rankq/margin_{name}"] = _masked_diagnostic(margin, success_mask)
