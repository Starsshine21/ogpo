...
class EpisodeMeta:
    ...
    bestofn_critic_checkpoint: str | None = None
    bestofn_group_size: int | None = None
    qgf_critic_checkpoint: str | None = None
    qgf_strength: float | None = None
...
def main() -> None:
    ...
    parser.add_argument('--bestofn-critic-checkpoint', type=Path)
    parser.add_argument('--bestofn-critic-config', type=Path)
    parser.add_argument('--bestofn-group-size', type=int, default=4)
    parser.add_argument('--bestofn-verify-base', action='store_true')
    parser.add_argument('--qgf-critic-checkpoint', type=Path)
    parser.add_argument('--qgf-critic-config', type=Path)
    parser.add_argument('--qgf-strength', type=float, default=1.0)
    parser.add_argument('--qgf-verify', action='store_true')
    ...
    if args.qgf_critic_checkpoint:
        if not args.episode_manifest or not args.qgf_critic_config:
            parser.error('QGF requires fixed episode manifest and critic config')
        if args.bestofn_critic_checkpoint or args.ogpo_checkpoint:
            parser.error('QGF is base-only and cannot combine with Best-of-N or OGPO actor')
    if args.bestofn_critic_checkpoint and not args.episode_manifest:
        parser.error('Best-of-N requires --episode-manifest with fixed policy noise seeds')
    ...
    if args.episode_manifest is not None:
        ...
        for index, meta in enumerate(existing_metas):
            ...
            if args.bestofn_critic_checkpoint is not None:
                if meta.get('bestofn_critic_checkpoint') != str(args.bestofn_critic_checkpoint.resolve()) or meta.get('bestofn_group_size') != args.bestofn_group_size:
                    raise ValueError('existing output used a different Best-of-N critic or group size')
            elif meta.get('bestofn_critic_checkpoint') is not None:
                raise ValueError('cannot resume Best-of-N output as base/actor evaluation')
            if args.qgf_critic_checkpoint is not None:
                if meta.get('qgf_critic_checkpoint') != str(args.qgf_critic_checkpoint.resolve()) or meta.get('qgf_strength') != args.qgf_strength:
                    raise ValueError('existing output used a different QGF protocol')
            elif meta.get('qgf_critic_checkpoint') is not None:
                raise ValueError('cannot resume QGF output with another policy')
    ...
    if args.bestofn_critic_checkpoint is not None and (args.ogpo_checkpoint is not None or args.bestofn_critic_config is None):
        raise ValueError('Best-of-N requires base policy and a critic config; cannot combine with actor checkpoint')
    if args.ogpo_checkpoint is not None:
        ...
        ogpo_root = Path(__file__).resolve().parents[1]
    ...
    if args.qgf_critic_checkpoint is not None:
        ...
        from robotwin_qgf import QGF
        qgf = QGF(args.qgf_critic_checkpoint, args.qgf_critic_config,
            args.output_dir/'qgf_audit', args.pi05_checkpoint_dir/'model.safetensors',
            strength=args.qgf_strength, verify=args.qgf_verify)
    if args.bestofn_critic_checkpoint is not None:
        sys.path.insert(0, str(Path(__file__).resolve().parents[1] / 'src'))
        from robotwin_bestofn import BestOfN
        bestofn = BestOfN(args.bestofn_critic_checkpoint,args.bestofn_critic_config,
            args.output_dir/'bestofn_candidates',args.pi05_checkpoint_dir/'model.safetensors',
            group_size=args.bestofn_group_size,verify_base=args.bestofn_verify_base)
    ...
    while collected < args.num_episodes:
        ...
        try:
            while task_env.take_action_cnt < task_env.step_lim and not task_env.eval_success:
                ...
                if qgf is not None:
                    policy_output = qgf.infer(model.policy,model.observation_window,observation,
                        task_env.get_instruction(),args.task_name,policy_noise_seed,policy_chunk_id,collected,
                        int(model.pi0_step),task_env.step_lim-task_env.take_action_cnt)
                elif bestofn is not None:
                    policy_output = bestofn.select(model.policy,model.observation_window,observation,
                        task_env.get_instruction(),args.task_name,policy_noise_seed,policy_chunk_id,collected,
                        int(model.pi0_step),task_env.step_lim-task_env.take_action_cnt)
                elif policy_noise_seed is None:
            ...
            meta = EpisodeMeta(
                ...
                bestofn_critic_checkpoint=(str(args.bestofn_critic_checkpoint.resolve()) if args.bestofn_critic_checkpoint else None),
                bestofn_group_size=(args.bestofn_group_size if bestofn is not None else None),
                qgf_critic_checkpoint=(str(args.qgf_critic_checkpoint.resolve()) if qgf is not None else None),
                qgf_strength=(args.qgf_strength if qgf is not None else None),
                ogpo_checkpoint=(
